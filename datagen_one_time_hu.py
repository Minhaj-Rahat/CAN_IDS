...
import numpy as np
import pickle
def gen_data(data_files, testFiles, config,author):
    ...
    X_train, X_test, y_train, y_test = train_test_split(x, Y, test_size=config[author]['dataGen']['TEST_DATA_PERCENTAGE'],
                                                        train_size=config[author]['dataGen']['TRAINING_DATA_PERCENTAGE'],
                                                        shuffle=True,
                                                        random_state=25)
    ...
    y_test = np_utils.to_categorical(y_test,config[author]['train']['NUM_OF_CLASSES'])

    # dump test file
    test_data = (X_test, y_test)
    f = open(testFiles[0], 'wb')
    ...
    X_train, X_test, y_train, y_test = train_test_split(x, Y,
                                                        test_size=config[author]['dataGen']['TEST_DATA_PERCENTAGE'],
                                                        train_size=config[author]['dataGen'][
